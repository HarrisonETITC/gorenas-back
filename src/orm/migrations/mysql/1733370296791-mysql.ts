...
export class Mysql1733370296791 implements MigrationInterface {
    name = 'Mysql1733370296791'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`usuario\` (\`id\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(200) NOT NULL, \`contrasena\` varchar(255) NOT NULL, \`estado\` varchar(1) NOT NULL DEFAULT 'A', \`fecha_creacion\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_2863682842e688ca198eb25c12\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`rol\` (\`id\` int NOT NULL AUTO_INCREMENT, \`nombre\` varchar(20) NOT NULL, \`estado\` varchar(1) NOT NULL DEFAULT 'A', \`fecha_creacion\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`fecha_modif\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_9792c580a992d554ee1621c5b4\` (\`nombre\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
