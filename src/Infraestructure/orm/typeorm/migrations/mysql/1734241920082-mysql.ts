...
export class Mysql1734241920082 implements MigrationInterface {
    name = 'Mysql1734241920082'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`branch\` (\`id\` int NOT NULL AUTO_INCREMENT, \`state\` varchar(1) NOT NULL DEFAULT 'A', \`name\` varchar(100) NOT NULL, \`address\` varchar(255) NULL, \`earnings\` decimal(16,2) NULL DEFAULT '0.00', \`created\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`modified\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`restaurant_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`person\` (\`id\` int NOT NULL AUTO_INCREMENT, \`names\` varchar(200) NULL, \`surnames\` varchar(200) NULL, \`identification\` varchar(15) NULL, \`type_identification\` varchar(4) NULL DEFAULT 'C.C', \`phone_number\` varchar(10) NULL, \`rh\` varchar(3) NULL DEFAULT 'O+', \`address\` varchar(255) NULL, \`born\` datetime NULL, \`created\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`user_id\` int NULL, \`role_id\` int NULL, UNIQUE INDEX \`REL_5157fa65538cae06e66c922c89\` (\`user_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`alter table branch modify name varchar(100) DEFAULT '';`);
    }
